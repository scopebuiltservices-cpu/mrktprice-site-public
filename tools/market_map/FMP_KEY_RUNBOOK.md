...
| --- | --- | --- |
| `overall: "no_key"` | The `FMP_ULTIMATE_API_KEY` secret is **not set** in this repo. | Add it (Step 4). |
...
existing one) → Name **`FMP_ULTIMATE_API_KEY`** → paste the key value → **Save**. Never put the key
in a URL, file, or commit — secrets only.
