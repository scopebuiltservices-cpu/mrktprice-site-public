...
External-enrichment pattern (like event_board.py): reads the SEC fails-to-deliver block already in
marketmap.json (n["short"].fails, from short_squeeze.py) plus the name's committed price/volume history
(hist/{T}.json rows [date,close,vol]) and market cap, then computes via the verified crowding_engine:
    days_to_cover = fails / ADV(21)                         # SEC FTD shares / avg daily volume  (REAL)
    siPct         = fails / denom                           # denom: REAL free float > shares-out proxy
    utilization   = clamp(dtc / 10)                         # borrow-tightness proxy until a lending feed exists
    penalty       = crowding_penalty(siPct, ownership=0, utilization)
and writes n["cr"] = {dtc, siPct, util, borrowFee, pen, squeeze, src}.

DENOMINATOR: FMP does NOT sell short interest, so the FTD numerator stays. But FMP Ultimate DOES sell real
FREE FLOAT (/stable/shares-float, via fmp_float.py -> float.json); float is the correct, much-smaller
denominator for low-float names, so the penalty engages where squeezes actually happen. Falls back to the
mcap/price shares-outstanding proxy when float.json is absent. Idempotent; verified. Research only."""
...
def crowding_for(short, rows, mcap, float_shares=None):
    ...
    # squeeze is a SINGLE-NAME concept: require a real equity short-interest % (si is None for ETFs).
