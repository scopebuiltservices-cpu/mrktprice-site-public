...
def compute_gex(chain, spot, days, r=0.04, contract=100):
    ...
    callGamma adds +, putGamma adds - to dealer gamma. Returns a compact dict, or None if no usable data."""
    if not chain or spot<=0: return None
    T = max(days,1)/365.0
    per = {}                                   # strike -> {cG,pG,cOI,pOI,iv list}
    cIVs=[]; pIVs=[]
    for o in chain:
        try:
        ...
        if g is None: g=bs_gamma(spot,K,T,iv if (iv and iv>0) else 0.3,r)
        d=per.setdefault(K,{"cG":0.0,"pG":0.0,"cOI":0.0,"pOI":0.0})
        if typ=="C": d["cG"]+=g*oi; d["cOI"]+=oi; (iv and iv>0) and cIVs.append((abs(K-spot),iv))
        else:        d["pG"]+=g*oi; d["pOI"]+=oi; (iv and iv>0) and pIVs.append((abs(K-spot),iv))
    if not per: return None
    Ks=sorted(per)
    # dealer gamma per strike (shares), and $GEX per 1% move
    dealer=[(per[K]["cG"]-per[K]["pG"])*contract for K in Ks]
    gex1=[g*spot*spot*0.01 for g in dealer]                       # $ per 1% move at each strike
    gex_total=sum(gex1)
    # total |gamma| OI per strike for walls (magnet strength), nearest above/below spot
    ...
    # zero-gamma flip: cumulative dealer $GEX across strikes, find sign change vs strike
    cum=[]; s=0.0
    for g in gex1: s+=g; cum.append(s)
    flip=_interp_zero(Ks, cum)
    # ATM IV (nearest-to-spot), expected move, put/call ratio, skew (OTM put IV - OTM call IV)
    atm_iv=None
    allIV=sorted(cIVs+pIVs)
    if allIV: atm_iv=allIV[0][1]
    exp_move=(atm_iv*math.sqrt(T)) if atm_iv else None             # fraction, ~1 straddle expected move
    cOI=sum(per[K]["cOI"] for K in Ks); pOI=sum(per[K]["pOI"] for K in Ks)
    pcr=round(pOI/cOI,2) if cOI>0 else None
    def _otm_iv(lst): return (sorted(lst)[len(lst)//2][1] if lst else None)  # mid-distance OTM proxy
    ...
    return {
        "gexTotal": round(gex_total),                              # $ dealer gamma per 1% move; >0 stabilizing
        ...
        "expMovePct": round(exp_move*100,2) if exp_move else None, # +/- % to expiry
