/* Exact Py<->JS parity for engine.js's deterministic estimators (EMA, rolling realized vol, OU AR(1),
   Lo-MacKinlay variance ratio). Loads the REAL engine.js + committed golden fixture engine_golden.json
   (from tools/market_map/engine_ref.py). Asserts engine.js reproduces the Python decimals to 1e-9.
   Run: node tools/test_engine_parity.mjs */
...
ok('ema parity', arrClose(E.ema(I.ema_c, I.ema_N), X.ema), 'ema mismatch');
