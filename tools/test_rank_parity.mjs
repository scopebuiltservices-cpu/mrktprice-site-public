...
ok('rank_engine.js exposes API', ['convictionSigma', 'lcbScore', 'compositeRankScore', 'deflatedConviction', 'grinoldKahn'].every(k => typeof R[k] === 'function'));
g.rows.forEach(function (row, i) {
  ...
  ok('lcb[' + i + ']', close(R.lcbScore(row.tot, cs, g.k), row.lcb), [R.lcbScore(row.tot, cs, g.k), row.lcb]);
  ok('score[' + i + ']', close(R.compositeRankScore(row.tot, row.z, row.base_sigma, g.k, g.n_tests), row.score));
